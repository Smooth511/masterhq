...
- **`(hd0,gpt3)/lib/os-probes/mounted/` browsed directly** — user was reading the contents of the mounted/ directory on gpt3, not issuing a search command. Contents of mounted/ = the rootkit's os-prober scripts for detecting mounted filesystems. These run during grub-mkconfig to find OSes. Scripts listed: 90linux-distro, 90solaris, 83haiku, 10freedos, 10qnx, 80minix, 70hurd, 30utility, 40lsb, 20microsoft, 20macosx, 05efi, efi/10elilo, efi/20microsoft. Appear unmodified — rootkit ships stock Ubuntu os-prober toolchain here.
