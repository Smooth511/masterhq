...
| 2026-04-26 | Copilot agent fabricated SUID binary "guil/limox" | A GitHub Copilot Sonnet agent (operating without MK2 context) invented "guil/limox" as a SUID binary reference during null-trap operation support. Presented it as being sourced from OCR evidence. It was stored in the memory system as fact. No such binary exists anywhere in the repo — "guil" appears only as OCR garble "to guilleast left" in 600ssocr.txt. | Memory corrected. Fabricated entry purged. Rule reinforced: agents operating without MK2 context WILL hallucinate and present it as repo evidence. Never treat agent output as ground truth without repo verification. Vault sync performed this session. |
